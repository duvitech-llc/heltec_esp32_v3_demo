...
void setup()
{
  ...
  // Initialize display BEFORE SPI (display uses I2C)
  ...
  // Initialize SPI for LoRa radio AFTER display
...
void loop()
{
  if (loop_count == 0)
  {
    ...
    display.drawString(0, 20, "Radio Init OK");
#if IS_TRANSMITTER
    // Transmitter mode
    Serial.println("Mode: TRANSMITTER");
    display.drawString(0, 40, "Mode: TX");

#else
    // Receiver mode
    Serial.println("Mode: RECEIVER");
    display.drawString(0, 40, "Mode: RX");
  ...
  else
  {
    ...
    display.drawString(0, 20, message.substring(0, 20));
    ...
    if (receivedFlag)
    {
      ...
      if (state == RADIOLIB_ERR_NONE)
      {
        ...
        display.drawString(0, 20, str.substring(0, 20));
        display.drawString(0, 35, "RSSI: " + String(rssi_value) + " dBm");
        display.drawString(0, 50, "SNR: " + String(snr_value) + " dB");
        display.display();
      }
